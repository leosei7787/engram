#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Dismantle the old AcmeCorp CPO Agent layout — archive Desktop/TOMTOM and
...
#   1. ~/engram-data/acmecorp/MEMORY/        exists and is non-empty
#   2. ~/engram-data/acmecorp/wiki/knowledge-base-wiki/ exists with 2000+ .md files
...
#
# Usage:
#   bash scripts/migrate-old-acmecorp.sh           # do it
#   bash scripts/migrate-old-acmecorp.sh --check   # validate prereqs only
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
if [ ! -d "$HOME/engram-data/acmecorp/MEMORY" ]; then
  echo "✗ ~/engram-data/acmecorp/MEMORY does not exist — cannot proceed"
  exit 1
fi
mem_size=$(du -sh "$HOME/engram-data/acmecorp/MEMORY" | cut -f1)
echo "✓ Memory at ~/engram-data/acmecorp/MEMORY ($mem_size)"

# 2. New wiki location
if [ ! -d "$HOME/engram-data/acmecorp/wiki/knowledge-base-wiki" ]; then
  echo "✗ ~/engram-data/acmecorp/wiki/knowledge-base-wiki does not exist"
  exit 1
fi
wiki_count=$(find "$HOME/engram-data/acmecorp/wiki/knowledge-base-wiki/wiki" -name "*.md" 2>/dev/null | wc -l | tr -d ' ')
echo "✓ Wiki at ~/engram-data/acmecorp/wiki/knowledge-base-wiki ($wiki_count .md files)"
...
if [ -f "$HOME/Library/LaunchAgents/com.acmecorp.wiki.sync.plist" ]; then
  echo "✗ Old com.acmecorp.wiki.sync launchd job is still active"
  echo "  Run: launchctl unload ~/Library/LaunchAgents/com.acmecorp.wiki.sync.plist"
  echo "       mv ~/Library/LaunchAgents/com.acmecorp.wiki.sync.plist{,.disabled}"
  exit 1
fi
